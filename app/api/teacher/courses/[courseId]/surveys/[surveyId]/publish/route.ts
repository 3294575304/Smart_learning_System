...
import {
  ...
  publishCourseSurveySchema,
} from "@/services/course-surveys/schemas";
import { publishCourseSurvey } from "@/services/course-surveys/service";
...
export async function POST(request: Request, context: Context) {
  ...
  const body = publishCourseSurveySchema.safeParse(
    await request.json().catch(() => null),
  );
  if (!courseId.success || !surveyId.success || !body.success)
    return apiError("问卷发布参数无效", 400);
  try {
    const teacher = await requireAuthenticatedUser([Role.TEACHER]);
    return apiSuccess(
      await publishCourseSurvey(
        teacher.id,
        courseId.data,
        surveyId.data,
