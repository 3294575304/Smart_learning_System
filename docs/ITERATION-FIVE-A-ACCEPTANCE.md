...
> 日期：2026-08-08；真实沙箱门禁关闭于 2026-08-09
>
> 范围：判题与画像共用的数据基础层
>
> 结论：A1–A5 与真实沙箱能力门禁均已通过，迭代五 A 完成；可以按既定范围进入五 B，但 Python 题型与正式判题仍未实现
...
- 独立 Ubuntu 22.04 x86_64 节点已部署 Docker 29.1.3、gVisor `runsc release-20260803.0`（`systrap`）和最小 Python 3.10.4 运行镜像；执行器最大并发为 1。
- 执行器以专用 `sandboxexec` 账户和加固 systemd 单元运行，API 只监听 `127.0.0.1:8788`，环境文件为 `0600 root:root`；本次通过 SSH 隧道完成验收，没有新增公网监听端口。
- `npm run test:sandbox-capabilities` 于 2026-08-09 实际执行并 8/8 通过，脱敏原始报告见 [`docs/evidence/iteration-five-a-sandbox-capabilities-2026-08-09.json`](evidence/iteration-five-a-sandbox-capabilities-2026-08-09.json)。
- 短任务的 Docker/gVisor 资源采样尚未产生非零 CPU、峰值内存和进程数；限制行为已通过攻击性探测，但五 B 在宣称精确资源计量前必须补齐更可靠的采样或 cgroup 读取。
...
| 真实沙箱限制                 | 通过 | 真实远程 gVisor 节点 8/8 探测通过并保存脱敏报告         |

## 范围边界

本阶段没有新增 Python 题型、代码编辑器、公开样例运行、隐藏用例评分、画像快照、AI 批量匹配或历史回填。真实隔离执行器的全部探测已经通过并保存验收报告，迭代五 A 门禁关闭；下一步可进入五 B，但不得把执行器基础误述为 Python 正式判题已经完成。
