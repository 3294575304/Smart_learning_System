...
> 结论：B0–B3、C、D、E 均已通过专项验收；最终结论和 2026-08-11 复跑证据见 `docs/ITERATION-FIVE-FINAL-ACCEPTANCE.md`。无人登录开机自启按部署目标选配，不作为功能专项验收门禁。
...
| 五 E  | 已通过 | 空库 migration、seed、HTTP/数据库集成、格式、lint、类型、全量测试、worker/执行器/Next.js 构建、浏览器人工核验、真实沙箱 8 项探测和远程判题纵向集成通过 |
...
1. 2026-08-11 核对时当前机器未安装 `Zhixue-*` 计划任务。仓库安装脚本已支持独立生产构建、失败重启和恢复用户活动时不停机；如要求无人登录运行，仍需改为专用服务账户或 Windows/Linux 服务。
2. 浏览器人工核验已覆盖教师编程题配置、公开/隐藏用例、课程图谱绑定、已发布判题结果、画像证据状态和非成员统一 404。为避免改写开发数据，本次没有再次代学生提交；正式提交链由独立 HTTP 纵向测试和 2026-08-10 远程真实集成共同证明。
...
迭代五整体已通过专项验收，下一业务优先级为课程图谱驱动推荐。正式部署前仍需按目标环境复核服务账户、密钥、ACL、自动恢复和真实安全证据。
