...
| V1.0 迭代五 B：Python 判题        | 已实现并通过远程验收 | 不可变配置、发布冻结、公开运行、隐藏用例异步判题、确定性评分、重试幂等、重判/撤销和证据投影已落地；本机 Next/PostgreSQL/worker → 远程 gVisor 纵向集成已通过 |
| V1.0 迭代五 C：画像与证据         | 已实现，待环境验收   | 不可变画像快照、输入指纹、事件水位、证据状态/置信度、出勤/活跃度/反思/客观掌握度分维度保存，以及师生证据下钻页面已落地                                      |
| V1.0 迭代五 D：批量题目映射       | 已实现               | 外部 AI 候选、严格输出/ID 范围校验、失败安全降级、教师预览/修改/确认、版本元数据和不覆盖人工绑定已落地；真实合成数据契约探测通过                            |
| V1.0 其余范围                     | 待开发               | 浏览器人工演示、无人登录开机自启、课程图谱联动推荐、自主练习/AI 自述、结课问卷、DOCX 报告和最终整体验收尚未完成                                             |

迭代四和迭代五 A 已完成专项验收。2026-08-10 已恢复专用节点授权、升级远程执行器并完成真实安全能力复测 8/8；本机 Next.js、PostgreSQL、登录态常驻 worker 和 SSH 隧道也已部署，Python 判题真实纵向集成覆盖公开运行、隐藏判题、确定性成绩及课程 Concept 证据投影。远程验收期间修复了进程限制错误分类和容器 stdin 未保留问题。浏览器人工演示和无人登录开机自启尚未完成，因此迭代五和 V1.0 仍不标记整体完成。
