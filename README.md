...
| V1.0 迭代六：课程图谱驱动推荐     | 开发中（第一批）     | 已增加版本化教学进度、默认推荐策略、课程推荐周期，以及课程/正式图谱/已授 Concept/题型/难度/近期练习硬过滤和结构化生成 API；练习证据回写、Python 推荐练习、AI 自述与界面仍待开发 |
| V1.0 迭代七及后续                 | 待开发               | 结课问卷、DOCX 报告和 V1.0 最终整体验收尚未完成                                                                                                                                 |

迭代五已于 2026-08-11 完成专项验收。2026-08-10 的远程 gVisor 安全能力复测 8/8 和真实 Python 判题纵向集成覆盖公开运行、隐藏判题、确定性成绩及课程 Concept 证据投影；2026-08-11 又通过 321/321 单测、44 个 migration 的空库 HTTP 集成、14/14 PostgreSQL 集成、三套生产构建和浏览器人工核验。Windows 无人登录开机自启按目标部署环境选配，不作为功能专项验收门禁。V1.0 仍未完成，当前优先开发课程图谱驱动推荐。

迭代二、三专项验收记录见 [`docs/ITERATION-TWO-THREE-ACCEPTANCE.md`](docs/ITERATION-TWO-THREE-ACCEPTANCE.md)，迭代四记录见 [`docs/ITERATION-FOUR-ACCEPTANCE.md`](docs/ITERATION-FOUR-ACCEPTANCE.md)，迭代五最终记录见 [`docs/ITERATION-FIVE-FINAL-ACCEPTANCE.md`](docs/ITERATION-FIVE-FINAL-ACCEPTANCE.md)。原有大纲异常消息契约已修复，固定样本合同测试已覆盖 3 项目标、11 章、8 个实验项目、32/20/12 学时和 10%/5%/5%/20%/60% 权重；最新全量质量检查结果以实际命令输出和验收记录为准。
...
迭代六第一批已建立课程推荐数据基座：教师通过 `GET/PUT /api/teacher/courses/:courseId/teaching-progress` 维护当前正式图谱的已授 Concept 范围，学生通过 `POST /api/student/courses/:courseId/recommendations` 提交知识点、题型、题量和难度条件。生成结果会冻结图谱、画像、掌握度、教学进度和策略版本，并只选择当前课程已确认绑定、已启用、有解析、难度匹配且近期未完成/未在进行中的题目；证据不足时推荐理由明确标注为诊断练习。旧 MVP 推荐历史不迁移、不改写。

下一批优先完成推荐练习结果到课程 `LearningEvent`/Concept 证据/掌握度修订/画像快照的追加式回写，再接入 Python 异步判题、自主练习页面和 AI 自述；其后依次开发结课问卷、DOCX 教学质量报告和 V1.0 整体界面/端到端验收。
