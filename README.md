...
当前状态摘要见 [`docs/PROJECT-STATUS.md`](docs/PROJECT-STATUS.md)，完整路线、范围和验收标准见 [`项目计划书.md`](项目计划书.md)。MVP 的角色权限、环境配置、演示流程和已知限制见 [`docs/MVP-ACCEPTANCE.md`](docs/MVP-ACCEPTANCE.md)。
...
| V1.0 迭代八：可靠性、整合与验收   | A 阶段部分实施       | 报告、图谱、大纲解析和题目映射已接入持久化独立 worker；安全与文件加固、三角色 E2E/CI、界面整合、可观测性、数据留存、部署恢复演练和 V1.0 总验收尚未完成           |
...
2026-09-07 重跑结果：386 项自动化测试中 385 项通过、1 项因未配置固定外部 PDF 路径跳过；PostgreSQL 推荐/通知集成 14/14 通过；Prisma 校验、开发库 54 个 migration 状态、格式、Lint、类型检查、Next.js 生产构建、沙箱执行器、判题 worker 和长任务 worker 构建均通过。空库 HTTP 集成能应用 54 个 migration 并通过认证、管理员和班级阶段，但大纲用例仍按旧同步响应断言，测试运行器未启动独立 worker，因而全套未通过；详情见 [`docs/PROJECT-STATUS.md`](docs/PROJECT-STATUS.md)。Windows 无人登录开机自启按目标部署环境选配，不作为功能专项验收门禁。
...
下一步先恢复异步长任务的 HTTP 集成编排，再完成迭代七登录态人工与真实 AI Provider 专项验收，并按《项目计划书》3.2 版继续推进迭代八 A—E：可靠任务与安全治理、自动验收与 CI、界面流程整合、可观测性/数据留存/部署演练以及 V1.0 总验收。
