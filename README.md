...
| V1.0 迭代五 A：任务与沙箱契约     | 已完成               | 通用任务、Attempt/租约/重试/取消、学习事件和远程执行器契约已落地；独立 gVisor 执行器已部署，真实安全能力探测 8/8 通过，可进入迭代五 B                  |
| V1.0 迭代五 B：Python 判题        | 已实现，待环境验收   | 不可变题目配置、发布冻结、公开运行、隐藏用例异步判题、确定性评分、重试幂等、重判/撤销、师生界面和成绩证据投影已落地                                    |
...
| V1.0 其余范围                     | 待开发               | 迭代五真实环境整体验收、课程图谱联动推荐、自主练习/AI 自述、结课问卷、DOCX 报告和最终整体验收尚未完成                                                  |

迭代四和迭代五 A 已完成专项验收。迭代五 B、C、D 的代码纵切已实现，外部 AI 合成数据契约探测通过；五 E 的空库 migration、权限和全量回归已通过。真实远程沙箱复测与故障恢复演练仍受专用节点 SSH 授权阻塞，V1.0 尚未完成整体验收。
