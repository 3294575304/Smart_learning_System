...
| V1.0 迭代六：课程图谱驱动推荐     | 开发中（第二批）     | 已增加版本化教学进度、课程推荐周期、题目—Concept 推荐快照，以及客观推荐练习到学习事件、追加式课程证据、掌握度修订和画像快照的事务闭环；Python 推荐练习、AI 自述与界面仍待开发 |
...
迭代六前两批已建立课程推荐数据基座和客观题反馈闭环：教师通过 `GET/PUT /api/teacher/courses/:courseId/teaching-progress` 维护当前正式图谱的已授 Concept 范围，学生通过 `POST /api/student/courses/:courseId/recommendations` 提交知识点、题型、题量和难度条件。生成结果冻结图谱、画像、掌握度、教学进度、策略版本及题目—Concept 来源；客观练习完成后在同一事务中追加课程学习事件和推荐证据，并生成新的掌握度修订与画像快照。旧 MVP 推荐历史不迁移、不改写，推荐练习不计入正式课程成绩。

下一批优先接入 Python 推荐异步判题与推荐证据重判/撤销，再完成自主练习页面和 AI 自述；其后依次开发结课问卷、DOCX 教学质量报告和 V1.0 整体界面/端到端验收。
