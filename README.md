...
| V1.0 迭代二：教学大纲解析         | 核心流程已完成，待固定样本整体验收   | 文本型 PDF 提取、AI 结构化解析、Zod 校验、来源引用、不可变审核修订、显式发布和历史版本已落地                                             |
| V1.0 迭代三：知识图谱             | 核心流程已完成，待整体验收与交互增强 | 正式大纲转图谱、AI `RELATED` 增强与降级、草稿/审核/发布、版本差异、历史版本和 PostgreSQL 节点/关系已落地                                 |
| V1.0 迭代五基础：题目映射与掌握度 | 部分完成                             | 教师可维护题目的课程级主/次知识点绑定；作业发布冻结绑定快照；自动/人工批改生成版本化证据和确定性掌握度修订，并提供师生权限隔离的查询 API |
| V1.0 其余范围                     | 待开发                               | 成绩与出勤台账、Python 基础判题、画像与掌握度界面、课程图谱联动推荐、自主练习/AI 自述、结课问卷、DOCX 报告和最终整体验收尚未完成         |

当前实际开发前沿已进入迭代五的基础能力，但迭代四“成绩与出勤台账”尚未实施，V1.0 也尚未完成整体验收。后续优先级以 [`项目计划书.md`](项目计划书.md) 和 [`AGENTS.md`](AGENTS.md) 为准。

当前质量基线：Markdown 格式、ESLint、TypeScript、Prisma Schema、生产构建和独立测试库 HTTP 集成测试均通过。`npm test` 串行复跑为 269 项中 268 项通过；唯一已知失败位于 `tests/courses/syllabus-parsing.test.ts`，原因是测试期望中文业务错误“结构化解析失败”，实际抛出英文 `AI output failed schema validation.`。修复前不得宣称全量单元测试通过。
...
- 当前教师工作区提供节点搜索、审核、发布、历史版本和版本差异接口；学生端图谱、缩放式图形交互、推荐联动、图数据库和 WebSocket/SSE 尚未实现。
