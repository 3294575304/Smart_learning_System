...
| V1.0 迭代五 B：Python 判题        | 已实现并通过远程验收 | 不可变配置、发布冻结、公开运行、隐藏用例异步判题、确定性评分、重试幂等、重判/撤销和证据投影已落地；独立 worker → 远程 gVisor 纵向集成已通过            |
...
| V1.0 其余范围                     | 待开发               | 生产应用内部地址与常驻 worker 部署、浏览器人工演示、课程图谱联动推荐、自主练习/AI 自述、结课问卷、DOCX 报告和最终整体验收尚未完成                      |

迭代四和迭代五 A 已完成专项验收。2026-08-10 已恢复专用节点授权、升级远程执行器并完成真实安全能力复测 8/8；Python 判题真实纵向集成也已通过，覆盖公开运行、隐藏判题、确定性成绩及课程 Concept 证据投影。远程验收期间修复了进程限制错误分类和容器 stdin 未保留问题。当前尚无生产应用内部 URL，常驻 worker 未部署，浏览器人工演示也尚未完成，因此迭代五和 V1.0 仍不标记整体完成。
